...
test.describe("Dashboard (Accueil)", () => {
	...
	test("should display dashboard tiles and lists correctly with mock data", async ({
		page,
	}) => {
		// Mock analyses
		...
		const analysesTile = page.locator('a[href="/analyses"]').first();
		await expect(analysesTile.locator("span.text-3xl")).toHaveText("2");

		// Recommandations Tile value is "new" count (1) — tuile avec to="/analyses" (même href que Analyses)
		const recosTile = page.locator('a[href="/analyses"]', {
			hasText: "Recommandations",
		});
		await expect(recosTile.locator("span.text-3xl")).toHaveText("1");

		// Documents Tile value is total length (2) — tuile avec to="/entreprises"
		const docsTile = page.locator('a[href="/entreprises"]', {
			hasText: "Documents",
		});
		...
		await expect(page.locator("text=Terminée").first()).toBeVisible();
		await expect(page.locator("text=Échec")).toBeVisible();

		// Recent recommendations
		await expect(page.locator("text=Optimiser le RAG")).toBeVisible();
		await expect(
			page.locator("text=Mettre en place des agents de support"),
		).toBeVisible();
	});
});
