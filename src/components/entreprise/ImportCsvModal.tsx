...
// Classe de la zone de dépôt selon l'état de survol / fichier sélectionné —
// table de correspondance plutôt que ternaires imbriquées.
function dropzoneClass(dragOver: boolean, file: File | null): string {
	if (dragOver) return "border-primary bg-primary/5";
	if (file) return "border-emerald-500 bg-emerald-500/5";
				...
				<div className="flex items-start gap-2 p-3 bg-emerald-500/8 rounded-xl border border-emerald-500/20">
					...
					<p className="text-xs text-muted-foreground">
						<strong className="text-foreground">
							{report.imported} entreprise{report.imported > 1 ? "s" : ""}
						</strong>{" "}
						ajoutée{report.imported > 1 ? "s" : ""} au portefeuille et enrichie
						{report.imported > 1 ? "s" : ""} automatiquement.
					</p>
				</div>
			)}
			...
			{report.skipped > 0 && (
				<div className="p-3 bg-muted/30 rounded-xl">
					<p className="text-xs font-semibold text-foreground mb-1">
						{report.skipped} doublon{report.skipped > 1 ? "s" : ""} ignoré
						{report.skipped > 1 ? "s" : ""}
					</p>
					<p className="text-xs text-muted-foreground">
						{report.skippedSirens.slice(0, 5).join(", ")}
						{report.skippedSirens.length > 5 ? "…" : ""}
					</p>
				</div>
			)}
				...
				<div className="p-3 bg-red-500/8 rounded-xl border border-red-500/20 space-y-1.5">
					...
					{report.errors.slice(0, 5).map((e) => (
						<div key={e.line} className="text-xs text-muted-foreground">
							<span className="font-mono text-foreground">Ligne {e.line}</span>{" "}
							— {e.message}
						</div>
					))}
					{report.errors.length > 5 && (
						<p className="text-xs text-muted-foreground">
							…et {report.errors.length - 5} autre
							{report.errors.length - 5 > 1 ? "s" : ""}
						</p>
					)}
				</div>
			)}
