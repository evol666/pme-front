...
import { useEntrepriseSearch, useEntreprise, type EntrepriseSearchResult } from "@/api/entreprises";
import {
  useAddEntreprise,
  RELATION_TYPES,
  type RelationType,
      ...
      <div className="w-full max-w-2xl bg-card border border-border rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-foreground">Ajouter une entreprise</h2>
              <p className="text-xs text-muted-foreground">
                Étape {step} sur 3 — {step === 1 ? "Recherche" : step === 2 ? "Confirmation" : "Relation"}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          {step === 3 && selected && (
            <Step3Relation kind={kind} setKind={setKind} notes={notes} setNotes={setNotes} />
          )}
        </div>
...
function Step1Search({ onSelect }: { onSelect: (r: EntrepriseSearchResult) => void }) {
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-1.5 text-xs text-muted-foreground">Minimum 2 caractères</p>
      </div>
        ...
        <div className="border border-border rounded-xl overflow-hidden divide-y divide-border/50">
          {results.map((r) => (
            ...
            >
              <span className={cn(
                ...
                r.etat === "A" ? "bg-emerald-500/10 text-emerald-600" : "bg-muted text-muted-foreground",
              )}>
              ...
              <span className="flex-1 min-w-0">
                ...
                <span className="block text-xs text-muted-foreground mt-0.5">
                  {r.siren}
                  {r.codeNaf && <> · {r.codeNaf}</>}
                  {r.etat !== "A" && <span className="ml-2 text-amber-500">cessée</span>}
                </span>
              </span>
          ...
          {data && data.total > results.length && (
            <div className="px-4 py-2 bg-muted/30">
              <p className="text-xs text-muted-foreground">
                {data.total.toLocaleString("fr-FR")} résultats — affinez pour en voir plus
              </p>
            </div>
          )}
        </div>
      ...
      {debounced.length >= 2 && !isFetching && results.length === 0 && (
        <div className="text-center py-8 text-muted-foreground">
          ...
          <p className="text-xs mt-1">Vérifiez l'orthographe ou saisissez le SIREN</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start gap-4 p-4 bg-muted/30 rounded-xl border border-border/50">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground mt-1 font-mono">{result.siren}</p>
          {id?.code_naf && (
        ...
        {score != null && (
          <div className={cn(
            ...
            severity === "faible" ? "bg-emerald-500/10 text-emerald-600" :
            severity === "modéré" ? "bg-amber-500/10 text-amber-500" :
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <div key={r.label} className="flex flex-col gap-0.5 px-3 py-2 bg-muted/20 rounded-lg">
              ...
              <span className="text-xs font-semibold text-foreground truncate">{r.value}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {id?.adresse && (
        <div className="flex items-start gap-2 text-xs text-muted-foreground">
          ...
          <span>{[id.adresse, id.code_postal, id.ville].filter(Boolean).join(", ")}</span>
        </div>
      )}
      ...
      <div className="flex items-start gap-2 p-3 bg-primary/5 rounded-lg border border-primary/20">
        ...
        <p className="text-xs text-muted-foreground">
          Ces données seront mises en cache. Cliquez <strong>Suivant</strong> pour définir la relation avec cette entreprise.
        </p>
      </div>
    </div>
  );
...
function Step3Relation({
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {RELATION_TYPES.map((r) => (
            ...
            >
              {r.label}
              {kind === r.value && <Check className="w-3.5 h-3.5 inline ml-1.5" />}
            </button>
          ))}
        </div>
      </div>
      ...
      <div>
        <label className="block text-sm font-semibold text-foreground mb-2">
          Notes internes <span className="font-normal text-muted-foreground">(optionnel)</span>
        </label>
        <textarea
          value={notes}
