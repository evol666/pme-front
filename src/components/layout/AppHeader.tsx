...
import { Bell, Clock, LogOut, Menu, ChevronDown, Users, Check } from "lucide-react";
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="hidden sm:inline-block max-w-[140px] truncate">{label}</span>
        ...
        <>
          ...
          <div className="absolute right-0 top-full mt-2 w-60 bg-card border border-border rounded-lg shadow-lg py-1 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
            <div className="px-3 py-2 border-b border-border">
              <p className="text-xs text-muted-foreground">Contexte d'affichage</p>
            </div>
            <button
              onClick={() => {
              ...
              className={cn(
                ...
                activePersonaId === null ? "text-foreground font-medium" : "text-muted-foreground",
              )}
            >
              <span className="flex-1 truncate">Tous les contextes</span>
              {activePersonaId === null && <Check className="w-4 h-4 text-primary" />}
            </button>
            {personas.map((persona) => (
              ...
              >
                <span className="flex-1 truncate">{persona.role ?? `Persona #${persona.id}`}</span>
                {activePersonaId === persona.id && <Check className="w-4 h-4 text-primary" />}
              </button>
            ))}
          </div>
        </>
