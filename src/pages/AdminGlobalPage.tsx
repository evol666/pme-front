...
type TabKey = "tenants" | "settings" | "profile" | "plan" | "branding" | "memory";
    ...
    <div className="space-y-8">
      <header className="space-y-3">
        ...
        <p className="max-w-2xl text-muted-foreground">
          Gestion multi-tenant : organisations, préférences, profil, plan, branding et
          mémoire. Réservé aux administrateurs plateforme.
        </p>
      </header>
...
function TenantsTab() {
  ...
  const handleDelete = async (t: Tenant) => {
    if (!window.confirm(`Supprimer le tenant « ${t.name} » ? Action définitive.`)) return;
    setError(null);
    try {
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        <LoadingState label="Chargement des tenants…" />
      ) : !data || data.length === 0 ? (
        <EmptyState
          icon={Users}
          title="Aucun tenant"
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {data.map((t) => (
            <TenantCard
              ...
              tenant={t}
              busy={
                (patchMutation.isPending && patchMutation.variables?.id === t.id) ||
...
function SettingsTab() {
  ...
  if (!data || data.length === 0)
    return (
      <EmptyState icon={Settings} title="Aucun paramètre" hint="Aucun réglage tenant enregistré." />
    );
  return (
    <div className="space-y-4">
    ...
    <article className="rounded-2xl border border-border bg-card p-5 shadow-sm space-y-3">
      <h3 className="text-sm font-semibold text-foreground">
        ...
        {settings.tenant.name && (
          <span className="ml-2 text-muted-foreground">· {settings.tenant.name}</span>
        )}
      </h3>
      <dl className="grid grid-cols-2 gap-2 text-xs">
        <Field label="Couleur primaire" value={settings.customPrimaryColor ?? "—"} />
        <Field label="Couleur secondaire" value={settings.customSecondaryColor ?? "—"} />
        ...
        <Field label="Personnalité IA" value={settings.aiPersonality ? "Définie" : "—"} />
      </dl>
      {features && (
...
function ProfileTab() {
  ...
  if (!data || data.length === 0)
    return <EmptyState icon={Eye} title="Aucun profil" hint="Aucun profil tenant enregistré." />;
  return (
    <div className="space-y-4">
      <RefetchButton isFetching={isFetching} onClick={() => refetch()} />
      <div className="grid gap-3 md:grid-cols-2">
...
function ProfileCard({ profile }: { profile: TenantProfile }) {
  ...
  return (
    <article className="rounded-2xl border border-border bg-card p-5 shadow-sm space-y-3">
      <h3 className="text-sm font-semibold text-foreground">
        ...
        {profile.tenant.name && (
          <span className="ml-2 text-muted-foreground">· {profile.tenant.name}</span>
        )}
      </h3>
      <dl className="grid grid-cols-2 gap-2 text-xs">
        ...
        <div className="col-span-2">
          ...
          <dd className="mt-0.5 text-foreground">{profile.primaryGoal ?? "—"}</dd>
        </div>
      </dl>
      {attrs && (
        <pre className="overflow-x-auto rounded-lg border border-border bg-background p-2 text-xs text-muted-foreground">
...
function PlanTab() {
  ...
  if (!data || data.length === 0)
    return <EmptyState icon={Sparkles} title="Aucun plan" hint="Aucun quota plan enregistré." />;
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {data.map((p) => (
          <PlanCard
            key={p.id}
            plan={p}
            busy={patchMutation.isPending && patchMutation.variables?.id === p.id}
            onToggle={() => toggle(p)}
          />
        ))}
      </div>
  ...
  return (
    <article className="rounded-2xl border border-border bg-card p-5 shadow-sm space-y-3">
      <header className="flex items-center justify-between">
        ...
        >
          {busy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : plan.isActive ? "Actif" : "Inactif"}
        </button>
      </header>
      <dl className="grid grid-cols-2 gap-2 text-xs">
        <Field label="Limite tokens/mois" value={plan.monthlyTokenLimit?.toLocaleString("fr-FR") ?? "—"} />
        <Field label="Limite requêtes/mois" value={plan.monthlyRequestLimit?.toLocaleString("fr-FR") ?? "—"} />
      </dl>
    </article>
  );
}
...
function BrandingTab() {
  ...
  if (!data || data.length === 0)
    return <EmptyState icon={Palette} title="Aucun branding" hint="Aucun branding tenant enregistré." />;
  return (
    <div className="space-y-4">
      <RefetchButton isFetching={isFetching} onClick={() => refetch()} />
      <div className="grid gap-3 md:grid-cols-2">
    ...
    <article className="rounded-2xl border border-border bg-card p-5 shadow-sm space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        {branding.primaryColor && (
          ...
          >
            <span className="h-2 w-2 rounded-full" style={{ backgroundColor: branding.primaryColor }} />
            Primaire
          </span>
        )}
...
function MemoryTab() {
  ...
  const { data, isLoading, isFetching, refetch } = useTenantMemories(category || undefined);
  const deleteMutation = useDeleteTenantMemory();
  const [error, setError] = useState<string | null>(null);

  const handleDelete = async (m: TenantMemory) => {
    if (!window.confirm(`Supprimer la mémoire « ${m.key} » ?`)) return;
    setError(null);
    try {
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        <LoadingState label="Chargement de la mémoire…" />
      ) : !data || data.length === 0 ? (
        <EmptyState icon={Brain} title="Aucune mémoire" hint="Aucune entrée mémoire tenant enregistrée." />
      ) : (
  ...
  return (
    <article className="rounded-2xl border border-border bg-card p-5 shadow-sm space-y-2">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="font-mono text-sm font-medium text-foreground">{memory.key}</span>
      ...
      <dl className="grid grid-cols-3 gap-2 text-xs text-muted-foreground">
        <Field label="Confiance" value={`${Math.round((memory.confidenceScore ?? 0) * 100)}%`} />
