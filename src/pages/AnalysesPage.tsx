...
// Couleurs du badge de statut — table de correspondance plutôt que ternaires
// imbriquées.
function statusBadgeClass(status: string): string {
	if (status === "completed") return "bg-emerald-500/10 text-emerald-600";
	if (["failed", "error"].includes(status)) return "bg-red-500/10 text-red-600";
...
export default function AnalysesPage() {
	...
	const handleLaunch = async (e: React.SubmitEvent) => {
		e.preventDefault();
					...
					<ul className="divide-y divide-border/50">
						{analyses.map((job) => (
							<li key={job.job_id}>
								<button
									type="button"
									aria-label={`Ouvrir l’analyse ${job.company_name ?? job.siren}`}
									className="w-full px-6 py-4 flex items-center gap-4 hover:bg-accent/30 transition-colors cursor-pointer text-left"
									onClick={() =>
