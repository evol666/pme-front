    ...
    <div className="space-y-8">
      <header className="space-y-3">
        ...
        <p className="max-w-2xl text-muted-foreground">
          Journal des événements instrumentés de la plateforme (event-store). Filtrez par
          nom d’événement ou catégorie.
        </p>
      </header>
          ...
          <td colSpan={6} className="px-4 py-3">
            <dl className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-3">
              <Detail
                label="Sujet"
                value={
                  [event.subjectKind, event.subjectId].filter(Boolean).join(" · ") || "—"
                }
              />
