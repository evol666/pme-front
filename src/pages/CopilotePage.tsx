...
function getHistoryMessage(
  ...
  if (key === "ArrowUp") {
    const nextIndex = currentIdx === null ? msgs.length - 1 : Math.max(0, currentIdx - 1);
    return { nextIndex, text: msgs[nextIndex] ?? "" };
  }
  if (key === "ArrowDown") {
    if (currentIdx === null) return null;
    if (currentIdx < msgs.length - 1) return { nextIndex: currentIdx + 1, text: msgs[currentIdx + 1] ?? "" };
  ...
  return (
    <li className="bg-card border border-border/50 rounded-xl p-3 space-y-2 shadow-sm">
      <div className="flex items-center gap-2">
        <span className={cn("text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full", severityTone(insight.severity))}>
          {insight.type}
        </span>
      ...
      <p className="text-[11px] text-muted-foreground leading-relaxed line-clamp-2">{insight.summary}</p>
...
function SuggestionRow({ suggestion }: { suggestion: CopilotSuggestion }) {
  const label =
    suggestion.kind === "recommendation" ? "Reco"
    : suggestion.kind === "priority" ? "Priorité"
    : suggestion.kind;
  return (
      ...
      <div className="min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          {suggestion.priority != null && (
            <span className="text-[9px] font-bold text-primary">P{suggestion.priority}</span>
          )}
        </div>
...
function ChatHistorySidebar({
  ...
  const visible = showArchived ? chats.filter((c) => c.archived) : chats.filter((c) => !c.archived);

  return (
    <div
      className={cn(
        "flex flex-col bg-card border border-border/50 rounded-2xl shadow-sm overflow-hidden",
        isMobile ? "fixed inset-y-0 left-0 z-50 w-72 rounded-none" : "w-64 shrink-0",
      )}
    >
      <div className="flex items-center justify-between p-4 border-b border-border/50">
        <h2 className="text-sm font-semibold">{showArchived ? "Archivées" : "Conversations"}</h2>
        <div className="flex gap-1">
          ...
          {isMobile && (
            <button type="button" onClick={onClose} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-accent">
              <X className="w-4 h-4" />
            </button>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
        {visible.length === 0 && (
          <p className="text-xs text-muted-foreground text-center py-8">
            {showArchived ? "Aucune conversation archivée." : "Aucune conversation. Cliquez sur + pour commencer."}
          </p>
        )}
        {visible.map((chat) => (
          <div
            ...
            role="button"
            tabIndex={0}
            onClick={() => { onChatSelect(chat.id); if (isMobile) onClose(); }}
            onKeyDown={(e) => { if (e.key === "Enter") { onChatSelect(chat.id); if (isMobile) onClose(); } }}
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-muted-foreground">
                {format(new Date(chat.lastActivity), "d MMM HH:mm", { locale: fr })}
              </p>
            </div>
            <button
              type="button"
...
export default function CopilotePage() {
  const { data: state, isLoading: stateLoading, refetch, isFetching } = useCopilotState({ maxSuggestions: 6, maxInsights: 5 });
  ...
  const { data: messages = [], isLoading: messagesLoading } = useChatMessages(currentChatId);
  ...
  return (
    <div className="space-y-4">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-widest text-primary">Assistant</p>
          <h1 className="text-2xl font-extrabold tracking-tight text-foreground mt-0.5 flex items-center gap-2">
            <Bot className="w-6 h-6 text-primary" />
            Copilote IA
          </h1>
          <p className="text-xs text-muted-foreground mt-0.5">
            Votre assistant contextuel. Il connaît votre entreprise, votre mémoire et vos analyses passées.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <HealthBadge />
        ...
        <section className="flex flex-col flex-1 min-w-0 bg-card border border-border/50 rounded-2xl shadow-sm overflow-hidden">
            ...
            {currentChatId && isEditingTitle ? (
              <div className="flex items-center gap-2 flex-1">
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      updateTitle.mutate({ chatId: currentChatId, title: newTitle });
                ...
                <button
                  type="button"
                  onClick={() => { updateTitle.mutate({ chatId: currentChatId, title: newTitle }); setIsEditingTitle(false); }}
              ...
              <div className="flex items-center gap-2 flex-1 min-w-0 group">
                <h2 className="text-sm font-semibold truncate text-foreground">
                  {currentChatId ? (currentChat?.title ?? "Chat") : "Sélectionnez ou créez une conversation"}
                </h2>
                {currentChatId && (
                  <button
                    type="button"
                    title="Renommer"
                    onClick={() => { setNewTitle(currentChat?.title ?? ""); setIsEditingTitle(true); }}
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-3">
            {!currentChatId && (
              <div className="h-full flex flex-col items-center justify-center text-center gap-3 py-12">
                ...
                <p className="text-sm font-medium text-foreground">Posez votre question au copilote</p>
                <p className="text-xs text-muted-foreground max-w-xs">
                  Ex. « Quelles actions prioritaires cette semaine ? », « Analyse ma situation de trésorerie ».
                </p>
                <button
                  type="button"
            ...
            {currentChatId && !messagesLoading && (
              <div className="flex justify-start">
                ...
                <div className="max-w-[78%] rounded-2xl rounded-bl-md px-3.5 py-2.5 text-sm leading-relaxed bg-accent text-foreground">
                  <p>Bonjour ! Je suis votre Copilote IA. Comment puis-je vous aider aujourd'hui ?</p>
                </div>
              </div>
            )}
              ...
              >
                ...
                >
                  ...
                  <span className="block mt-1 text-[10px] opacity-60">
                    {m.createdDate && !Number.isNaN(new Date(m.createdDate).getTime())
          ...
          <div className="border-t border-border/50 p-3 flex items-center gap-2 shrink-0">
            <textarea
              ...
              onKeyDown={(e) => {
                ...
                if (e.key === "ArrowUp" || e.key === "ArrowDown") {
                  const res = getHistoryMessage(e.key, historyIndexRef.current, userMessageTexts);
                  if (res) {
                    setInput(res.text);
              ...
              rows={1}
              placeholder={currentChatId ? "Écrivez votre message… (Entrée pour envoyer)" : "Sélectionnez une conversation pour commencer"}
            ...
            <button
              type="button"
              onClick={handleSend}
              disabled={!input.trim() || !currentChatId || sendMessage.isPending}
          ...
          {stateLoading ? (
            ...
            <>
              <div className="bg-card border border-border/50 rounded-2xl p-4 shadow-sm">
                ...
                {insights.length === 0 ? (
                  <p className="text-xs text-muted-foreground py-1">Aucune alerte active.</p>
                ) : (
              ...
              <div className="bg-card border border-border/50 rounded-2xl p-4 shadow-sm">
                ...
                {suggestions.length === 0 ? (
                  <p className="text-xs text-muted-foreground py-1">Lancez une analyse pour générer des suggestions.</p>
                ) : (
