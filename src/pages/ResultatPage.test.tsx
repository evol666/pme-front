...
const propositionStructuree = JSON.stringify({
	...
	recommendations: [
		...
		{
			name: "Renégocier les délais",
			rationale: "Auprès des fournisseurs.",
			priority: "medium",
		},
	...
	actionPlan: [
		{
			titre: "Cartographier les encours",
			description: "Par client.",
			duree: "1 semaine",
		},
		{
			name: "Mettre en place les relances",
			summary: "Automatiques.",
			duration: "2 semaines",
		},
	],
...
describe("Livrable — garde sur le job", () => {
	...
	it("renvoie vers la liste des analyses", () => {
		...
		fireEvent.click(
			screen.getByRole("button", { name: "Retour aux analyses" }),
		);
...
describe("Livrable — en-tête", () => {
	it("titre le livrable au nom de l’entreprise et à son métier", () => {
		...
		expect(
			screen.getByRole("heading", { name: "Translog" }),
		).toBeInTheDocument();
	...
	it("retombe sur l’identifiant de job quand l’entreprise est inconnue", () => {
		...
		expect(
			screen.getByRole("heading", { name: "job-123" }),
		).toBeInTheDocument();
	});
...
describe("Livrable — rendu brut", () => {
	it("verse le texte libre dans le résumé quand ce n’est pas du JSON", () => {
		brancher({
			data: analyse({ proposal: "Proposition rédigée en texte libre." }),
		});

		afficher();

		expect(
			screen.getByText("Proposition rédigée en texte libre."),
		).toBeInTheDocument();
