    ...
    <div className="space-y-8">
      <header className="space-y-3">
        ...
        <p className="max-w-2xl text-muted-foreground">
          État de l’IA locale, alertes proactives, indicateurs clés et consommation des
          modèles. Données issues du backend Spring Boot.
        </p>
      </header>
...
function HealthCard({ health }: { health: PmeHealth }) {
  const ollamaTone =
    health.ollama_status === "ready"
      ? "bg-emerald-500/10 text-emerald-600"
      : health.ollama_status === "model_missing" || health.ollama_status === "unreachable"
  ...
  return (
    <article className="rounded-2xl border border-border bg-card p-6 shadow-sm space-y-5">
      <div className="grid gap-3 sm:grid-cols-2">
        <StatusPill label="Backend" value={health.backend_status} tone={backendTone} />
        <StatusPill
        ...
        <div className="space-y-3">
          {alerts.map((a) => (
            <AlertCard
              key={a.id}
              alert={a}
              busy={patchMutation.isPending && patchMutation.variables?.id === a.id}
              onSeen={() => patch(a, { status: "seen", seenAt: new Date().toISOString() })}
              onActed={() =>
                patch(a, { status: "acted", actedAt: new Date().toISOString() })
              }
              onDismiss={() =>
                patch(a, { status: "dismissed", dismissedAt: new Date().toISOString() })
              }
            />
          ))}
        </div>
  ...
  return (
    ...
    >
        ...
        <div className="min-w-0 space-y-1.5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">
              {acted ? "Traitée" : dismissed ? "Écartée" : alert.status}
            </span>
          ...
          {alert.summary && (
            <p className="text-sm text-muted-foreground line-clamp-3">{alert.summary}</p>
          )}
        </div>
      ...
      <dl className="grid grid-cols-2 gap-2 text-xs text-muted-foreground sm:grid-cols-4">
        <Field label="Confiance" value={`${Math.round((alert.confidence ?? 0) * 100)}%`} />
      ...
      {alert.relatedSiren && (
        <p className="text-xs text-muted-foreground">SIREN lié : {alert.relatedSiren}</p>
      )}
    </article>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-border bg-card p-4 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          <label className="flex-1 space-y-1.5">
            <span className="text-sm font-medium text-foreground">Indicateur</span>
            <input
              type="text"
              value={kpi}
          ...
          <label className="space-y-1.5">
            <span className="text-sm font-medium text-foreground">Granularité</span>
            <input
              type="text"
              value={granularity}
      ...
      {isLoading ? (
        ...
        <EmptyState icon={Gauge} title="Aucun KPI" hint="Aucun instantané enregistré." />
      ) : (
...
function KpiCard({ kpi }: { kpi: KpiSnapshot }) {
  ...
  return (
    <article className="rounded-2xl border border-border bg-card p-5 shadow-sm space-y-3">
      ...
      <h3 className="font-mono text-sm font-semibold text-foreground">{kpi.kpi}</h3>
      ...
      <dl className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
        ...
        <Field label="Précédent" value={kpi.valuePrev?.toLocaleString("fr-FR") ?? "—"} />
...
function UsageRow({ usage }: { usage: AiUsage }) {
  const statusTone =
  ...
  return (
    <tr className="hover:bg-accent/40">
      <td className="px-4 py-3 font-medium text-foreground">{usage.provider}</td>
      <td className="px-4 py-3 font-mono text-xs text-foreground">{usage.model}</td>
